      ...
      if (line.startsWith("GSE1: ")) {
        ...
        if (command.equals("INITIALIZE")) {
          if (!systemInitialized) {
            ...
            //Valve Diameter Calc
            float D_n_init = ((5.02e-6) * psi_sq_init) + ((8.23e-5) * psi_init) + (4.38e0);
            float D_e_init = ((1.77e-6) * psi_sq_init) + ((2.06e-4) * psi_init) + (2.09e0);
            //Valve Turn Calc
...
void loop() {
  ...
  if (millis() - lastUpdate >= updateInterval) {
    ...
    float D_n = ((5.02e-6) * psi_sq) + ((8.23e-5) * psi) + (4.38e0);
    float D_e = ((1.77e-6) * psi_sq) + ((2.06e-4) * psi) + (2.09e0);
