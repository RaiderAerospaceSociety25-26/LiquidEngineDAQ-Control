      ...
      if (line.startsWith("GSE1: ")) {
        ...
        if (command.equals("INITIALIZE")) {
          if (!systemInitialized) {
            ...
            //Valve Diameter Calc
            float D_n_init = ((3.88e-6) * psi_sq_init) + ((2.55e-4) * psi_init) + (3.95e0);
            float D_e_init = ((2.23e-6) * psi_sq_init) + ((2.76e-4) * psi_init) + (2.69e0);
            //Valve Turn Calc
...
void loop() {
  ...
  if (systemRunning && (millis() - lastUpdate >= updateInterval)) {
    ...
    float D_n = ((3.88e-6) * psi_sq) + ((2.55e-4) * psi) + (3.95e0);
    float D_e = ((2.23e-6) * psi_sq) + ((2.76e-4) * psi) + (2.69e0);
