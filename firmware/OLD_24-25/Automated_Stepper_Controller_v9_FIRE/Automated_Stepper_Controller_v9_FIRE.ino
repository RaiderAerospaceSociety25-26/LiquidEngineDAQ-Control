...
const unsigned long updateInterval = 750;  // Update every 750ms <-------------------------------------|
...
const unsigned long dataTransmissionInterval = 1500; //Sends Valve Data every 1 seconds <------------------| 
      ...
      if (line.startsWith("GSE1: ")) {
        ...
        if (command.equals("INITIALIZE")) {
          if (!systemInitialized) {
            ...
            //Valve Diameter Calc
            float D_n_init = ((3.71e-6) * psi_sq_init) + ((2.30e-4) * psi_init) + (3.74e0);
            float D_e_init = ((1.34e-6) * psi_sq_init) + ((1.62e-4) * psi_init) + (1.61e0);
            //Valve Turn Calc
            ...
            Serial.println("InitialStep_n: "); Serial.println(InitialStep_n);
...
void loop() {
  ...
  if (millis() - lastUpdate >= updateInterval) {
    ...
    float D_n = ((3.71e-6) * psi_sq) + ((2.30e-4) * psi) + (3.74e0);
    float D_e = ((1.34e-6) * psi_sq) + ((1.62e-4) * psi) + (1.61e0);
  ...
  if (systemRunning && (millis() - lastDataTransmission >= dataTransmissionInterval)) {
    // Display results in Arduino Serial Monitor
    Serial.println("=====================================");
    Serial.print("PSI: "); Serial.print(global_psi, 2); Serial.println(" pis");
    ...
    Serial.print("Nitrous Last Turns: "); Serial.print(global_Last_T_n_ref, 3); Serial.println(" turns");
    Serial.print("Ethanol Last Turns: "); Serial.print(global_Last_T_e_ref, 3); Serial.println(" turns");
    Serial.print("Nitrous Current Turns: "); Serial.print(global_T_n, 3); Serial.println(" turns");
    Serial.print("Ethanol Current Turns: "); Serial.print(global_T_e, 3); Serial.println(" turns");
